...
export async function chat(messages) {
        ...
        if (role === 'user' && (attachUrl || dataUrl)) {
            try {
                ...
                if (dataUrl) {
                    ...
                    if (m) { mimeType = m[1]; buf = Buffer.from(m[2], 'base64') }
                    else buf = bufferFromDataUrl(dataUrl)
                } else if (attachUrl) {
                    const resp = await axios.get(attachUrl, { responseType: 'arraybuffer', timeout: 45000 })
    ...
    if (DEBUG) {
        try { console.log('[vertex:req]', JSON.stringify({ count: contents?.length || 0, hasSystem: !!systemParts.length })) } catch {}
    }
    ...
    if (DEBUG) {
        try { console.log('[vertex:open]') } catch {}
    }
