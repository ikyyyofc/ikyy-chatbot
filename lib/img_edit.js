...
export async function imgEditUrl(imageBuffer, prompt) {
  ...
  if (DEBUG) console.log('[tool:edit_image:upload_ok]', JSON.stringify({ pollingUrl }))
  ...
  while (Date.now() - start < 120000) {
    ...
    if (DEBUG) console.log('[tool:edit_image:poll]', JSON.stringify({ status }))
    if (status === 'Ready') {
  ...
  if (DEBUG) console.log('[tool:edit_image:done]', JSON.stringify({ resultUrl }))
  return resultUrl
}
