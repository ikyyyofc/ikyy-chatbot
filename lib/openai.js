...
async function generate_image({ prompt, size = '1024x1024', style, aspect_ratio }) {
  ...
  try {
    // Use anabot text2imageFlux API as requested
    const { data } = await axios.get('https://anabot.my.id/api/ai/text2imageFlux', {
      params: { prompt: safePrompt, apikey: 'freeApikey' },
      headers: { accept: 'application/json' },
      timeout: 60000
    })

    // Extract image URL robustly across possible shapes
    const url = (
    ...
    return JSON.stringify({ status: 'error', provider: 'anabot', error: String(msg) })
  }
}
