...
export async function chat(messages) {
  ...
  const toolInstruction = {
    role: 'developer',
    content: [
      'STRICT REAL-TIME POLICY:',
      '- Before answering ANY user question, ensure all factual statements are up-to-date RIGHT NOW.',
      '- Use get_current_datetime({ timeZone?, locale? }) to read the exact current date/time (default timeZone: Asia/Jakarta for Indonesian context).',
      '- Use realtime_info(query) to fetch/verify ANY facts that may have changed since training (news, prices, schedules, stats, releases, events, weather, etc.).',
      '- Do NOT rely on internal knowledge for time-sensitive facts; prefer tool outputs. If a tool fails or times out, explain the limitation instead of guessing.',
      '- Only skip realtime_info when the task is purely non-factual/self-contained (e.g., poetry, rewrite user-provided text, refactor code provided in-context).',
      '- When facts are used, cite URLs from sources and include a brief note like: "Diperbarui per: <timestamp> (<TZ>)".'
    ].join(' ')
  }
