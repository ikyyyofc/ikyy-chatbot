  ...
  async function sendAndStream(msgs, depth = 0) {
    ...
    resp.data.on('data', (chunk) => {
      ...
      while ((idx = buf.indexOf('\n')) !== -1) {
        ...
        try {
          ...
          // buffer content until we know whether a tool call will happen
          if (delta?.content) pendingText += delta.content
    ...
    return await new Promise((resolve, reject) => {
      resp.data.on('end', async () => {
        ...
        if (sawToolCall) {
          // drop any pending text from the pre-tool phase to avoid premature answers
          pendingText = ''
