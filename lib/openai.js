...
export async function chat(messages) {
  ...
  async function sendAndStream(msgs, depth = 0) {
    ...
    const resp = await axios.post('https://api.openai.com/v1/chat/completions', body, { responseType: 'stream', headers: authHeaders })
      ...
      resp.data.on('end', async () => {
        ...
        if (sawToolCall) {
          ...
          for (const tc of toolCalls) {
            ...
            nextMsgs.push({ role: 'tool', tool_call_id: tc.id, name: tc.function.name, content: result })
          }
