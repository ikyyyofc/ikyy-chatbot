...
// Streaming chat dengan DeepSeek (Chat Completions SSE)
// DeepSeek API kompatibel dengan OpenAI format
...
export async function chat(messages) {
  ...
  // Gunakan DEEPSEEK_API_KEY, fallback ke OPENAI_API_KEY jika tidak ada
  const apiKey = process.env.DEEPSEEK_API_KEY || await key() || process.env.OPENAI_API_KEY
  if (!apiKey) throw new Error('Missing DEEPSEEK_API_KEY or OPENAI_API_KEY')

  const working = [...messages]
  const out = new PassThrough()

  // Ubah authorization header untuk DeepSeek
  const authHeaders = {
  ...
  async function sendAndStream(msgs) {
    // Gunakan model DeepSeek, fallback ke OPENAI_MODEL jika tidak ada DEEPSEEK_MODEL
    const model = process.env.DEEPSEEK_MODEL || OPENAI_MODEL || 'deepseek-chat'
    ...
    if (DEBUG) {
      try {
        console.log('[deepseek:req]', JSON.stringify({ model: model, msgCount: body.messages.length }))
      } catch {}
    }

    let resp
    try {
      // Ubah endpoint ke DeepSeek API
      resp = await axios.post('https://api.deepseek.com/chat/completions', body, { 
        responseType: 'stream', 
        headers: authHeaders 
      })
    } catch (e) {
      ...
      if (status === 401) {
        const err = new Error('DeepSeek unauthorized: missing/invalid DEEPSEEK_API_KEY')
        err.status = 401
        throw err
      }
      const detail = e?.response?.data?.error?.message || e?.message || 'request failed'
      const err = new Error(`DeepSeek request failed (${status ?? 'no_status'}): ${detail}`)
      err.status = status
      throw err
    }
    ...
    return await new Promise((resolve, reject) => {
      resp.data.on('end', async () => {
        ...
        if (sawToolCall) {
          ...
          if (DEBUG) {
            try { console.log('[deepseek:tools_detected]', JSON.stringify({ tools: toolCalls.map(t => t.function?.name) })) } catch {}
          }
