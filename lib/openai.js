...
async function generate_image({ prompt, size = '1024x1024', style, aspect_ratio }) {
  ...
  try {
    // Many Wudysoft endpoints respond with raw image bytes.
    const resp = await axios.get('https://wudysoft.xyz/api/ai/txt2img/gen/v6', {
      params: { prompt: safePrompt },
    ...
    if (status >= 400) {
      ...
      throw new Error(`http_${status}${detail ? ':' : ''} ${detail}`)
    }

    let url = ''
    if (/application\/(json|ld\+json)|text\//i.test(ct)) {
      // JSON or text response — try parse and extract link
      let txt = ''
      ...
      url = typeof candidate === 'string' ? candidate : ''
      if (!/^https?:\/\//i.test(url)) {
        // deep search string http(s)
        const stack = [data]
        const seen = new Set()
        while (stack.length && !url) {
          ...
          for (const k of Object.keys(v)) {
            const val = v[k]
            if (typeof val === 'string' && /^https?:\/\//i.test(val)) { url = val; break }
      ...
      // Binary image — save to /uploads and return server URL
      url = await saveImageToUploads(buf, ct)
    } else {
      // Unknown content-type — try to guess image; if small, parse text
      if (buf.length > 0 && buf[0] === 0x89 && buf[1] === 0x50 && buf[2] === 0x4e && buf[3] === 0x47) {
        url = await saveImageToUploads(buf, 'image/png')
      } else if (buf.length > 1 && buf[0] === 0xff && buf[1] === 0xd8) {
        url = await saveImageToUploads(buf, 'image/jpeg')
      } else {
        let txt = ''
        try { txt = buf.toString('utf8') } catch {}
      ...
      if (!url) throw new Error('unknown_response_type')
    }
    ...
    return JSON.stringify({ status: 'error', provider: 'wudysoft', error: String(msg) })
  }
}
            ...
            try {
              ...
              if (tc.function.name === 'realtime_info') {
                ...
                try {
                  ...
                  if (DEBUG) console.log('[tool:edit_image:args]', JSON.stringify({ hasParsedImageUrl: !!parsed?.image_url, hasParsedBase64: !!parsed?.image_base64, hasAttachUrlFromMsg: !!attachUrlFromMsg, hasDataUrlFromMsg: !!dataUrlFromMsg, prompt: sanitizeText(prompt, 80) }))
                ...
                if (!buf) {
                  ...
                  try {
                    ...
                    result = JSON.stringify({ status: 'ok', provider: 'easemate', url, prompt: sanitizeText(prompt, 100) })
                  } catch (e) {
                    if (DEBUG) console.error('[tool:edit_image:error]', e?.message || e)
                    result = JSON.stringify({ status: 'error', provider: 'easemate', error: String(e?.message || 'edit_failed') })
                  }
                }
              } else {
