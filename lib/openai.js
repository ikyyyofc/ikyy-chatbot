...
async function realtime_info({ query }) {
  ...
  const { text: gpt, source: sources } = await (await axios.get(`https://beta.anabot.my.id/api/ai/perplexity?prompt=${encodeURIComponent(q)}&apikey=freeApikey`)).data.data.result
