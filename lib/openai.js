...
function convertMessages(msgs) {
  ...
  if (DEBUG) {
    try {
      const stats = { total: outMsgs.length, multimodal: outMsgs.filter(x => x && x.role==='user' && Array.isArray(x.content)).length }
      console.log('[openai:convertMessages]', JSON.stringify(stats))
    } catch {}
  }
  return outMsgs
}
